...
namespace Gigascapes.SystemDebug
{
    ...
    public class RplidarMesherMap2B : MonoBehaviour
    {
        ...
        void Start()
        {
			...
			//Debug.Log(RplidarBinding2B.GetLDataSize());

			//Debug.Log(RplidarBinding2B.GetSampleData());

			//Debug.Log(RplidarBinding2B.OnConnectC(COM).ToString());

			RplidarBinding2B.OnConnectC(COM);
            RplidarBinding2B.StartMotorC();
            m_onscan = RplidarBinding2B.StartScanC();
        ...
        void OnDestroy()
        {
            ...
            RplidarBinding2B.EndScanC();
            RplidarBinding2B.EndMotorC();
            RplidarBinding2B.OnDisconnectC();
            RplidarBinding2B.ReleaseDriveC();
        ...
        void GenMesh()
        {
            while (true)
            {
                int datacount = RplidarBinding2B.GetDataC(ref m_data);
                if (datacount == 0)
                {
                    Thread.Sleep(20);
                }
